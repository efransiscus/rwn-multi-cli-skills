...
description: Architecture rules for multi-agent delegation — read-only orchestrator delegates mutations to specialized subagents (coder, reviewer). Covers agent roles, write-path restrictions (.ai/.kiro/.kimi/.claude only for orchestrator), failure handling, delegation flow, and per-CLI implementation notes. Use when setting up agents, reviewing delegation behavior, or debugging orchestrator/subagent interactions.
---
