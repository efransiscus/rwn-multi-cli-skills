...
description: Architecture rules for multi-agent delegation — read-only orchestrator delegates mutations to 12 specialized subagents (see agent-catalog for the roster). Covers write-path restrictions (.ai/.kiro/.kimi/.claude only for orchestrator), scoped-executor vs diagnoser classes, failure handling, delegation flow, and per-CLI implementation notes. Use when setting up agents, reviewing delegation behavior, or debugging orchestrator/subagent interactions.
---
